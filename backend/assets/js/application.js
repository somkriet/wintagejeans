...
$(".chooseproduct").change(function (e) {
    ...
    //         name_product = "POLO";
    //         $('.product_name_data').text(name_product);
   ...
   function chooseproduct_new(cate_price,cate_num){

        $(".product_name_data").empty();
        // console.log('product_name :' + name_product);
        ...
        var product_id = parseInt(cate_num);
        changeProduct(product_id);
        loadGradeByProductID(product_id);
...
$(".choosefabaic").change(function (e) {
    ...
    // var name_fabric = $(".choosefabric_new").parent().find("button").attr("grade_name");
    // $('.fabric_name_data').text(name_fabric);

 get_fabric(Canvas.product_id,Canvas.grade_id);

 function choosefabaic_newfabric(production_day,min_quantity,grade_price,grade_id){
    $('.fabric_name_data').empty();

    Canvas.current_grade_id = false;
    Canvas.min_quantity = min_quantity;
    Canvas.grade_price = grade_price;
    if (!Canvas.grade_price) {
        Canvas.grade_price = 0;
    }
    POLO.grade_color = TSHIRT.grade_color = APRON.grade_color = BAG.grade_color = $(".choosefabaic_new").parent().find("button").attr("grade_color");

    Canvas.grade_id = grade_id;
    changeProduct(Canvas.product_id);
    loadGradeByProductID(Canvas.product_id);
    get_fabric(product_id,grade_id);
   
    $('.fabaic_show').modal('hide');
}
...
function changeProduct(product_id) {
    ...
    $(".choosefabaic_new").val(Canvas.grade_id);
    $(".chooseproduct_new").val(product_id);
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var key in data) {
                ...
                var fabaic = '<div class="col-md-3 col-xs-2" align="center">'+
                                '<div class="card" style="width: 27rem;">'+
                                  '<img width="200px" class="card-img-top content_application" src="'+ baseURL+url_image +'" alt="Card image cap">'+
                                  // '<a href="'+ baseURL+url_image +'" class="product-material-view"><img src="'+ baseURL+url_image +'" alt="" class="img-responsive img-rounded"></a>'+
                                  '<div class="card-body">'+
                                    '<h5 class="card-title">'+ grade_name +'</h5>'+
                                    '<button class="btn btn-primary btn-sm choosefabaic_new"  value="'+grade_id+'"  onclick="choosefabaic_newfabric('+production_day+','+min_quantity+','+grade_price+','+grade_id+');">Choose this fabric</button>'+
                                  '</div>'+
                                '</div><br>'+
                            '</div>'
                $(".choosefabaicshow").append(fabaic);
            }


            get_fabric(product_id,Canvas.current_grade_id);
...
function get_fabric(product_id,grade_id){
    $.ajax({
        url: baseURL + 'Apps/getGradeByProductIDAndGradeID',
        type: 'POST',
        data: { 'product_id': product_id,
                'grade_id': Canvas.current_grade_id
              },
        dataType: 'json',
        success: function (json) {
           
                console.log(JSON.stringify(json)); // แสดงข้อมูลที่คืนค่ามาในหน้าต่าง console
                $('.fabric_name_data').text(json['grade_name']);
                Canvas.grade_price = json['grade_price'];
                Canvas.min_quantity = json['min_quantity'];
                Canvas.production_day = json['production_day'];
                POLO.grade_color = TSHIRT.grade_color = APRON.grade_color = BAG.grade_color = json['grade_color'];
